...
const unsigned long TIMER_5MIN = 60000; //300000
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (client && client.connected()) {
      ...
      pagina += "<div class='card'><h2>APAGADO AUTOMATICO</h2><div>Estado: <span id='r4'></span></div><div>Tiempo restante: <span id='timer'>0 seg</span></div></div>";
      pagina += "<div class='card'><h2>LUZ INTERNA</h2><div>Estado: <span id='r3'></span></div><button onclick='rele3On()'>ENCENDER</button><button onclick='rele3Off()'>APAGAR</button></div>";
      pagina += "</body></html>";
