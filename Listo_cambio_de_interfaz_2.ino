...
//  - KY-013 (temperatura interna) + ajuste +1°C (adaptado para Vcc=5V con divisor)
...
// -------------------------------------------------------------
//  PARÁMETROS Y AJUSTES PARA KY-013 (5V con DIVISOR)
//  Si el KY está alimentado a 5V, NUNCA conectes su salida directamente al ADC del ESP32.
//  Debes usar un divisor que reduzca 0..5V a 0..~3.3V. Aquí asumimos que existe ese divisor.
//  Ajusta R_DIV_TOP y R_DIV_BOTTOM según tu circuito de hardware.
// -------------------------------------------------------------
const float SENSOR_VCC = 5.0f;    // KY alimentado a 5V
const float ADC_REF = 3.3f;       // referencia aproximada usada para convertir ADC->V (ver nota)
const int ADC_MAX = 4095;         // resolución 12-bit del ESP32
const int ADC_ATTENUATION = ADC_11db; // usamos 11db para aproximar rango 0..~3.3V-3.6V

// Si usas divisor: sensor_out -- R_DIV_TOP -- ADC_PIN -- R_DIV_BOTTOM -- GND
// Vadc = Vsensor * R_DIV_BOTTOM/(R_DIV_TOP + R_DIV_BOTTOM)
// Para mapear 5V -> ~3.3V usamos por ejemplo R_DIV_TOP=10000, R_DIV_BOTTOM=20000
const float R_DIV_TOP = 10000.0f;     // resistencia superior del divisor (entre sensor y ADC)
const float R_DIV_BOTTOM = 20000.0f;  // resistencia inferior del divisor (entre ADC y GND)
const bool USE_VOLTAGE_DIVIDER = true; // poner true si efectivamente colocaste el divisor

// Parámetros del termistor / módulo KY-013 (ajusta si tienes valores distintos)
const float R_SERIES = 10000.0f;   // resistencia fija del módulo / divisor interno (ajusta si es distinto)
const float R_NOMINAL = 10000.0f;  // resistencia nominal a T0 (10k)
const float BETA = 3950.0f;        // coeficiente beta típico
const float T0_K = 298.15f;        // 25°C en Kelvin

// Offset de calibración para alinear KY con DHT (puedes ajustar desde Serial o web)
...
void setup() {
  ...
  // Configurar ADC del ESP32 para KY-013
  analogReadResolution(12);                   // 12-bit: 0..4095
  analogSetPinAttenuation(KY_PIN, ADC_ATTENUATION);  // atenuación para rango mayor (aprox 0..3.3-3.6V)
...
void loop() {
  ...
  if (IrReceiver.decode()) {
    ...
    if (rawCode == 0xB847FF00) { //B847FF00
      modo = 3;
      EEPROM.writeInt(addrModo, modo);
      EEPROM.commit();
      digitalWrite(RELAY1_PIN, LOW);
  ...
  // Leemos KY-013 con función que considera divisor y Vcc=5V
  float tempKY = readKY013Temperature();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (client) {
      ...
      pagina += "});}";
      pagina += "setInterval(actualizar,1000);";
...
// -------------------------------------------------------------
// Función: readKY013Temperature
// Descripción:
//  - Lee N muestras del ADC del KY_PIN
//  - Convierte la lectura del ADC a voltaje medido en el pin (Vadc)
//  - Si existe divisor, reconstruye la tensión original en el sensor (Vsensor)
//  - Calcula la resistencia del termistor usando Vsensor y SENSOR_VCC (5V)
//  - Aplica la fórmula Beta para obtener la temperatura en ºC
//  - Aplica calibrationOffset para ajuste manual
// -------------------------------------------------------------
float readKY013Temperature() {
  ...
  int rawKY = suma / muestras;

  // Voltaje medido en el pin ADC
  float v_adc = ((float)rawKY * ADC_REF) / (float)ADC_MAX;

  // Si no estamos usando divisor (NO RECOMENDADO si sensor a 5V), asumimos Vsensor = v_adc
  float v_sensor = v_adc;

  if (USE_VOLTAGE_DIVIDER) {
    // Reconstruir V_sensor (antes del divisor):
    // v_adc = v_sensor * R_DIV_BOTTOM / (R_DIV_TOP + R_DIV_BOTTOM)
    float factor = (R_DIV_TOP + R_DIV_BOTTOM) / R_DIV_BOTTOM; // >1
    v_sensor = v_adc * factor;
  }

  // Seguridad: evitar valores imposibles y advertir si ADC saturado
  if (v_adc >= (ADC_REF * 0.98f)) {
    Serial.println("ADVERTENCIA: lectura ADC cercana al tope. Revisa divisor o alimentacion (posible saturacion).");
  }
  if (v_sensor <= 0.0001f) v_sensor = 0.0001f; // evitar division por cero

  // Calcular Rtermistor suponiendo divisor R_SERIES y termistor entre Vcc y nodo intermedio:
  // V_sensor = SENSOR_VCC * Rtherm / (R_SERIES + Rtherm)
  // => Rtherm = R_SERIES * (V_sensor / (SENSOR_VCC - V_sensor))
  float denom = (SENSOR_VCC - v_sensor);
  if (denom <= 0.0001f) denom = 0.0001f; // evitar division por cero y lecturas invalidas

  float rTherm = R_SERIES * (v_sensor / denom);

  // Aplicar fórmula Beta -> temperatura en Kelvin
  float invT = (1.0f / T0_K) + (1.0f / BETA) * log(rTherm / R_NOMINAL);
  float tempK = 1.0f / invT;
  float tempC = tempK - 273.15f;

  // Aplicar offset de calibración en ºC
  tempC += calibrationOffset;

  // Depuración en Serial
  Serial.print("RAW_KY: "); Serial.print(rawKY);
  Serial.print(" | Vadc: "); Serial.print(v_adc, 3);
  Serial.print(" V | Vsensor: "); Serial.print(v_sensor, 3);
  Serial.print(" V | Rtherm: "); Serial.print(rTherm, 1);
  Serial.print(" ohm | TempKY_C: "); Serial.println(tempC, 2);
